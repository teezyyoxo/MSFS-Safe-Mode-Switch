...
**Make sure you update the sim path values in the script as needed for your install.** If you are using the `.exe`, rebuild it with `build.ps1` after making your modifications to the script so the packaged version includes your updated paths.
...
On the next launch of the sim with the `running.lock` file in place, the user will be presented with the following dialog box:

<img width="667" height="429" alt="image" src="https://github.com/user-attachments/assets/51d9b865-4d58-4016-9850-7db6a4c21bcb" />

(image courtesy of MSFS FAQ, linked below)
...
4. For the **executable version**, use [distribution/msfsSafeModeSwitch.exe](./distribution/msfsSafeModeSwitch.exe). If you need different default paths, edit the script first and then rebuild the `.exe`.
